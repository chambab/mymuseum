...
public class AbstractDAO {
     ...
     *        table name
     * @param targetObj
     *        including the needed inputted name at
     *        query execution
     ...
     *        table name
     * @param targetMap
     *        Map including the needed inputted value
     ...
     * including the inputted value. At the execution
    ...
    protected Object findByPk(String tableName, Object[] targetObjs)
        ...
        Collection collection =
            queryService.find(findPrefix + tableName + findByPkPostfix,
                targetObjs, 0, 0);
     ...
     *        table name
     * @param targetMap
     *        the Map including the needed inputted
     *        value at the query execution
     * @return Object containing the query result
     * @throws QueryServiceException
     *         if there is any problem executing the
     ...
     *         query
     */
    protected Collection findList(String tableName, List targetList,
            int pageIndex, int pageSize) throws QueryServiceException {
        Object[] params = convertParams(targetList);
        return findList(tableName, params, pageIndex, pageSize);
    }
     ...
     * info and Object array including the inputted
     * value, after finding the appropriate queryId in
     * combining the inputted table name and defined
     * findPrefix(default='find') and
     ...
     *         query
     */
    private Collection findList(String tableName, Object[] targetObjs,
            int pageIndex, int pageSize) throws QueryServiceException {
        return queryService.find(findPrefix + tableName + findListPostfix,
            targetObjs, pageIndex, pageSize);
    }
     ...
     *         query
     */
    protected Page findListWithPaging(String tableName, Object targetObj,
            int pageIndex, int pageSize) throws QueryServiceException {
     ...
     *        table name
     * @param targetObj
     *        the object including the inputted name at
     *        the query execution
     * @param pageIndex
     *        page number which expected to be
     *        displayed.
    ...
    protected Page findListWithPaging(String tableName, Object targetObj,
            ...
            throws QueryServiceException {
        Object[] params = convertParams(targetObj);
        return findListWithPaging(tableName, params, pageIndex, pageSize,
            pageUnit);
    }
     ...
     *         query
     */
    protected Page findListWithPaging(String tableName, Map targetMap,
            int pageIndex, int pageSize) throws QueryServiceException {
     ...
     * Transmit by changing as anyframe.common.Page
     * object handled the paginavigator tage for
     * supporting ease at the developgin of UI for the
     * execution result (page unit is set as 10.) In
     ...
     *        table name
     * @param targetList
     *        the List including the inputted object at
     *        the query exeuction
     * @param pageIndex
     *        page number which expected to be
     *        displayed.
     ...
     *         query
     */
    protected Page findListWithPaging(String tableName, List targetObjs,
            int pageIndex, int pageSize) throws QueryServiceException {
        return findListWithPaging(tableName, targetObjs, pageIndex, pageSize,
            10);
    }
     ...
     * combining the inputted table name, defined
     * findPrefix(default='find') and
     * findListPostfix(default='List'). Trasmit by
     ...
     * findPrefix(default='find') and
     * findListPostfix(default='List'). Transmit by
     * chaning as anyframe.common.Page object type
     * handled in the paginavigator tag for supporting
     ...
     * @param targetObj
     *        the Object including the needed inputted
     *        value at the query execution
     ...
     * @param targetObj
     *        the Map including the inputted value
     ...
     * @param targetList
     *        the List including the needed inputted
     *        value at query execution
    ...
    private Object[] convertParams(List targetList) {
        ...
        for (int i = 0, size = targetList.size(); i < size; i++) {
            params[i] =
                new Object[] {variableName + (i + 1), targetList.get(i) };
        }
        return params;
    }
}
